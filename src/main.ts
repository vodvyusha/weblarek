...
events.on("preview:change", () => {
  ...
  const cardElement = previewTemplate.content.firstElementChild?.cloneNode(
    true,
  ) as HTMLElement;
...
events.on("basket:item-delete", ({ product }: { product: IProduct }) => {
  cartModel.removeProduct(product);
  renderBasket();
});
